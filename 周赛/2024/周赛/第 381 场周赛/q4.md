...
当时折腾了一天，以为会有不错的表现，结果运行时间有一千七八百多 ms，只击败了11%。做了一点点修改（不用自带的 `max` 和 `min` 等）后，提升到了`1300+ms`，击败了33%左右。
...
学习了其他大佬的差分的方法，感觉还是[小羊的方法](https://leetcode.cn/problems/count-the-number-of-houses-at-a-certain-distance-ii/solutions/2613400/xiao-yang-xiao-en-dui-cheng-qing-kuang-d-i72g)好一些。学习和注释了一下。
